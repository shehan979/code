...
function filterByRadius(center, radiusKm = 50) {
    ...
    // 🧹 1️⃣ Close all currently open popups BEFORE filtering
    ...
    // 🔁 3️⃣ Rebuild clusters for visible markers
    if (clusterer) clusterer.clearMarkers();
    ...
    // 🎯 4️⃣ Focus map on radius area (after popups closed)
    if (visibleCount > 0 && !bounds.isEmpty()) {
      setTimeout(() => {
        map.fitBounds(bounds);
      }, 200); // small delay ensures no popup interferes
    } else {
